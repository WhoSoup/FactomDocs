...
| ----------------- | ---------------- | ------- | --------- |
| 0x00 | Minute Number | 1 byte | (Deprecated in Milestone2) The preceding data was acknowledged before the minute specified. 1 byte follows the Minute Number. |
...
| 0x08 | Add Federated Server Signing Key | 69 bytes | This adds an Ed25519 public key to the authority set.  First 32 bytes are the server's identity ChainID.  Next byte is the key priority.  Next 32 bytes are the public key itself.  Next 4 bytes are the Directory Block height that it takes effect.  If the specified key for this server already exists, this replaces the old one. |
...
| 0x0B | Coinbase Descriptor | 10232 bytes max | This is a field which is used to specify a future genesis transaction.  This field is only present every 25 blocks, on blocks divisible by 25. The coinbase transaction which is created by the info in this transaction included 1000 blocks after this Admin block. This delay is to allow 7 days to respond to software bugs, etc. See the **Coinbase Descriptor** section for more details. |
...
| 32 bytes | BodyMR | This is the Merkle root of the Factoid transactions which accompany this block.  The leaves of the Merkle tree are the full Factoid transaction, from the version through the last signature, inclusive. Minute markers are also leaves of this tree. It is calculated with SHA256. |
